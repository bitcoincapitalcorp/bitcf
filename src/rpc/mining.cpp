...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateBits(fGenerate, nGenProcLimit, Params());
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated FirstBitcoinCapitalCorp to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "FirstBitcoinCapitalCorp is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "FirstBitcoinCapitalCorp is downloading blocks...");
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks     (numeric, does nothing - backward compatability with FirstBitcoinCapitalCorp RPC)\n"
            "\nResult:\n"
