...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop FirstBitcoinCapitalCorp server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "FirstBitcoinCapitalCorp server stopping";
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current FirstBitcoinCapitalCorp address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid FirstBitcoinCapitalCorp address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid FirstBitcoinCapitalCorp address:")+s.name_);
...
Value gettxlistfor(const Array& params, bool fHelp)
{
    ...
    if (!searchAddress.IsValid())
        throw runtime_error("<address> is not valid FirstBitcoinCapitalCorp address\n");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; FirstBitcoinCapitalCorp server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "FirstBitcoinCapitalCorp is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "FirstBitcoinCapitalCorp is downloading blocks...");
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: BIT-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use bitcfd";
